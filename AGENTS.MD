...
Nestor owns this. Start: say hi + 1 motivating line.
Work style: telegraph; noun-phrases ok; drop grammar; min tokens.

## Agent Protocol

- Contact: Nestor Lopez (@nstlopez, nestor@nstlopez.com).
- Workspace: `~/git`. Missing nstlopez repo: clone `https://github.com/nsttt/<repo>.git`.
- 3rd-party/OSS (non-nstlopez): clone under `~/git`.
- “MacBook” / “Mac Studio” => SSH there; find hosts/IPs via `tailscale status`.
- Files: repo or `~/git/agents`.
...
- Web: search early; quote exact errors; prefer 2024–2025-2026 sources; fallback Firecrawl (`pnpm mcp:*`) / `mcporter`.
...
- Blog repo: `~/git/my-portfolio`
- OpenAI/Codex limits tracking: `~/Documents/steipete/codex limits.md`
- Obsidian vault: `$HOME/Documents/Nsttt Vault`

## Docs

- Start: run docs list (`docs:list` script, or `~/.codex/scripts/docs-list` here if present; ignore if not installed); open docs before coding.
- Global `docs-list` on PATH (`~/.local/bin/docs-list` -> `~/.codex/bin/docs-list`); rebuild: `bun build ~/.codex/scripts/docs-list.ts --compile --outfile ~/.codex/bin/docs-list`.
- Follow links until domain makes sense; honor `Read when` hints.
- Keep notes short; update docs when behavior/API changes (no ship w/o docs).
- Add `read_when` hints on cross-cutting docs.
- Model note (2025-11-23): no `gpt-5.1-pro` / `grok-4.1` on Nestor’s keys yet.
...
- Remotes under `~/git`: prefer HTTPS; flip SSH->HTTPS before pull/push.
- Commit helper on PATH: `~/.codex/scripts/committer` (bash). Prefer it; if repo has `./scripts/committer`, use that.
...
- Rust: use `cargo`; run `docs:list`; keep files small; follow existing patterns.
...
Read `~/git/agents/tools.md` for the full tool catalog if it exists.

### bird

- X CLI: `~/git/bird/bird`. Cmds: `tweet`, `reply`, `read`, `thread`, `search`, `mentions`, `whoami`.
- Uses Firefox cookies by default (`--firefox-profile` to switch).

### peekaboo

- Screen tools: `~/git/Peekaboo`. Cmds: `capture`, `see`, `click`, `list`, `tools`, `permissions status`.
- Needs Screen Recording + Accessibility. Docs: `~/git/Peekaboo/docs/commands/`.
...
### mcporter / iterm2 / firecrawl

- MCP launcher: `npx mcporter <server>` (see `npx mcporter --help`). Common: `iterm2`, `firecrawl`.
