...
- Every consuming repo’s `AGENTS.MD` is reduced to the pointer line `READ ~/git/agents/AGENTS.MD BEFORE ANYTHING (skip if missing).` Place repo-specific rules **after** that line if they’re truly needed.
...
## @nstlopez Agent Instructions (pointer workflow)
